...
class Node:
    def __init__(self, id, label, properties):
        ...
        self.properties = {
            k: escape_neo4j_string(v) if isinstance(v, str) else v
    ...
    def create_node(self):
        if (
            not self.id
            or not self.label
            or any(value is None for value in self.properties.values())
        ):
...
class Edge:
    ...
    def create_edge_query(self):
        props = ", ".join([f"{key}: ${key}" for key in self.properties.keys()])
        return (
            f"MATCH (a {{id: '{self.from_node_id}'}}), (b {{id: '{self.to_node_id}'}}) "
...
class Case:
    def __init__(self, data):
        ...
        self.decided_date = datetime.fromtimestamp(
            data.get("timeline", [{}])[0].get("dates", [0])[0]
        )
        self.citation = Citation(data.get("citation", {}))
        self.advocates = (
            [Advocate(a) for a in data.get("advocates", []) if a]
            if data.get("advocates")
            else []
        )
        self.decisions = (
            [Decision(d) for d in data.get("decisions", []) if d]
            if data.get("decisions")
            else []
        )
        self.decided_by = (
            DecidedBy(data.get("decided_by", {})) if data.get("decided_by") else None
        )
        self.term = data.get("term")
        self.justia_url = data.get("justia_url")
        self.written_opinion = (
            [WrittenOpinion(o) for o in data.get("written_opinion", []) if o]
            if data.get("written_opinion")
            else []
        )
...
def save_opinion_embedding(case_id, opinion_text):
    ...
    for chunk in chunks:
        ...
        metadata = {"case_id": str(case_id), "chunk": chunk}
        ...
        for opinion in written_opinions:
            if opinion:
                opinion_node = Node(
                    opinion.id,
                    "Opinion",
                    {"title": opinion.title, "case_id": case_node.id},
                )
                opinion_node.create_node()
                case_opinion_edge = Edge(
                    case_node.id, opinion_node.id, "case_opinion", {}, "count"
                )
                ...
                if entities:
                    entities_nodes = [
                        entity_to_node(entity) for entity in entities if entity
                    ]
                    for relation in relations:
                        if relation:
                            relation_nodes, relation_edge = relation_to_nodes_and_edges(
                                relation
                            )
                    ...
                    for node in entities_nodes:
                        if node:
                            node.create_node()
                            mentioned_in_edge = Edge(
                                case_node.id, node.id, "mentioned_in", {}, "count"
                            )
...
def process_scotus_case(case: Case):
    ...
    justices = (
        case.decided_by.members if case.decided_by and case.decided_by.members else []
    )
    ...
    case_node = Node(
        ...
        "Case",
        {
            ...
            "term": case.term,
            "decided_date": case.decided_date.strftime("%Y-%m-%d"),
        },
    )
    ...
    if first_party:
        ...
        case_party_edge_1 = Edge(
            case_node.id, first_party_node.id, first_party_node_name, {}, "count"
        )
    ...
    if second_party:
        ...
        case_party_edge_2 = Edge(
            case_node.id, second_party_node.id, second_party_node_name, {}, "count"
        )
    ...
    for advocate in advocates:
        advocate_node = Node(
            advocate.name,
            "Advocate",
            {"name": advocate.name, "description": advocate.description},
        )
        advocate_node.create_node()
        advocate_edge = Edge(
            case_node.id, advocate_node.id, "advocated_by", {}, "count"
        )
    ...
    for decision in decisions:
        if decision.winning_party:
            decision_node = Node(
                decision.winning_party, "Party", {"name": decision.winning_party}
            )
